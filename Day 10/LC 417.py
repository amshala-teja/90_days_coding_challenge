...
# You are given an m x n integer matrix heights where heights[r][c] represents the height above sea level of the cell 
# at coordinate (r, c).
...
class Solution(object):
    def pacificAtlantic(self, heights):
        ...
        for j in range(n):
            p_que.append((0, j))
            p_seen.add((0, j))
        #left, skip the left top corner 1
        for i in range(1, m):
            p_que.append((i, 0))
            p_seen.add((i, 0))
        #for the bottom
        for j in range(n-1):
            a_que.append((m-1, j))
            a_seen.add((m-1, j))
        #for the right part excluding the bottom right corner
        for i in range(m):
            a_que.append((i, n-1))
            a_seen.add((i, n-1))
